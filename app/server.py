...
from app.pipeline.setup_check import (
...
class LMStudioModelRequest(BaseModel):
    model: str


class OllamaModelRequest(BaseModel):
    """Legacy compatibility request model during LM Studio migration."""
    model: str
...
# Temporary compatibility alias for the existing frontend.
...
@app.get("/api/settings/lmstudio-model")
def get_lmstudio_model_setting() -> dict:
    return {
...
@app.post("/api/settings/lmstudio-model")
def set_lmstudio_model_setting(payload: LMStudioModelRequest) -> JSONResponse:
    try:
        model = set_lmstudio_model(payload.model)
    except ValueError as exc:
        return JSONResponse(status_code=400, content={"error": str(exc)})
    return JSONResponse({"model": model})


# Temporary compatibility aliases for the existing frontend.
